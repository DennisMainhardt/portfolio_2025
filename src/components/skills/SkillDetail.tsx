...
const SkillDetail = ({ skill, isVisible }: SkillDetailProps) => {
  ...
  const getSeparatorClass = () => {
    switch (skill.color) {
      case "electric-blue":
        return "from-electric-blue/30 via-electric-blue/10 to-transparent";
      case "plasma-violet":
        return "from-plasma-violet/30 via-plasma-violet/10 to-transparent";
      case "neon-green":
        return "from-neon-green/30 via-neon-green/10 to-transparent";
      case "yellow-400":
        return "from-yellow-400/30 via-yellow-400/10 to-transparent";
      default:
  ...
  return (
    <div className="pt-2 pb-6">
      <div className={`h-px mx-6 mb-6 bg-gradient-to-r ${getSeparatorClass()}`} />
      <div className="px-6 space-y-6">
        {/* Focus Areas */}
        <div>
          <div className="flex items-center gap-3 mb-3">
            <Target className={`w-4 h-4 text-${skill.color}`} />
          ...
          <div className="grid grid-cols-2 gap-2">
            {content.focusAreas.map((area, index) => {
              ...
              return (
                ...
                >
                  <div
                    className={`flex-shrink-0 w-7 h-7 flex items-center justify-center rounded-full bg-${skill.color}/10`}
                  >
                    <IconComponent className={`w-4 h-4 text-${skill.color}`} />
                  </div>
        ...
        <div>
          <div className="flex items-center gap-3 mb-3">
            <TrendingUp className={`w-4 h-4 text-${skill.color}`} />
